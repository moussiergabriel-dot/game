...
import { CUSTOM_SHAPE_KIND, Mathf, OBJECT_COLLISION_KIND, SHAPE_KIND } from '../Common/index.js';
import { Data } from '../index.js';
import { CustomGeometry } from './CustomGeometry.js';
import { Object3D } from './Object3D.js';
function buildUnitTriangles(geo) {
    ...
    for (let i = 0; i < pos.count; i++) {
        vertices.push(new THREE.Vector3((pos.getX(i) - center.x) / size.x, (pos.getY(i) - center.y) / size.y, (pos.getZ(i) - center.z) / size.z));
        uvs.push(uv ? new THREE.Vector2(uv.getX(i), uv.getY(i)) : new THREE.Vector2(0, 0));
    }
    return { vertices, uvs };
}
const geometryCache = new Map();
function getUnitTriangles(shapeKind, segments) {
    ...
    updateGeometry(geometry, position, count) {
        ...
        if (this.datas.collisionKind === OBJECT_COLLISION_KIND.SIMPLIFIED) {
            ...
            objCollision.push({
                ...
                m: Math.max(Math.ceil(w / 2 / Data.Systems.SQUARE_SIZE), Math.max(Math.ceil(h / 2 / Data.Systems.SQUARE_SIZE), Math.ceil(d / 2 / Data.Systems.SQUARE_SIZE))),
                k: true,
            });
        }
